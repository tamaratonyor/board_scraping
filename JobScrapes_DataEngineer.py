...
try:
	...
	for searchval in paramlist:
		...
		if len(indeedlinklist) == len(indeedtitlelist) == len(indeedlocationlist) == len(indeedcompanylist) == len(indeedrealdatelist):		
			dfindeed = pd.DataFrame({'Date': indeedrealdatelist,'Job_Title': indeedtitlelist, 'URL' : indeedlinklist,'Company': indeedcompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : indeedcitylist, 'State': indeedstatelist, 'Department' : "Big Data"})
		...
		if len(linkedinlinklist) == len(linkedintitlelist) == len(linkedinlocationlist) == len(linkedincompanylist) == len(linkedindatelist):
			dflinkedin = pd.DataFrame({'Date': linkedindatelist,'Job_Title': linkedintitlelist, 'URL' : linkedinlinklist,'Company': linkedincompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : linkedincitylist, 'State': linkedinstatelist, 'Department': "Big Data"})
		...
		if len(monsterlinklist) == len(monstertitlelist) == len(monsterlocationlist) == len(monstercompanylist) == len(monsterrealdatelist):
			dfmonster = pd.DataFrame({'Date': monsterrealdatelist,'Job_Title': monstertitlelist, 'URL' : monsterlinklist,'Company': monstercompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : monstercitylist, 'State': monsterstatelist, 'Department': "Big Data"})
		...
		if len(simplylinklist) == len(simplytitlelist) == len(simplylocationlist) == len(simplycompanylist) == len(simplydatelist):
			dfsimply = pd.DataFrame({'Date': simplydatelist,'Job_Title': simplytitlelist, 'URL' : simplylinklist,'Company': simplycompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : simplycitylist, 'State': simplystatelist, 'Department': "Big Data"})
