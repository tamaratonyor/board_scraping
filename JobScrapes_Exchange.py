...
if strdate == '1,9': #Drop old records on the 1st of the month at 9:00 am
	...
	sql = "DROP TABLE IF EXISTS JobPostings_Exchange"
	mycursor.execute(sql) 

paramlist = ["Exchange%20Engineer","Messaging%20Engineer","Unified%20Communications%20Engineer","Office%20365%20Engineer","Microsoft%20Messaging%20Engineer", "skype%20Engineer", "Systems%20Engineer", "Windows%20Engineer", "UC%20Engineer", "Mail%20Engineer", "Teams%20Engineer", "Mail%20Migration%20Engineer", "Enterprise%20Voice%20Engineer", "Exchange%20Architect", "Active%20Directory%20Architect", "Office%20365%20Architect", "Messaging%20Architect", "Exchange%20Administrator", "Messaging%20Administrator", "Microsoft%20Messaging%20Administrator" , "Skype%20Adminitrator" , "UC%20Administrator", "Teams%20Administrator", "Exchange%20Consultant", "Messaging%20Consultant", "Office%20365%20Consultant"]

try:
	df = pd.read_sql_table('JobPostings_Exchange', database_connection)
	if 'level_0' in df:
	...
	for searchval in paramlist:
		...
		if len(indeedlinklist) == len(indeedtitlelist) == len(indeedlocationlist) == len(indeedcompanylist) == len(indeedrealdatelist):		
			dfindeed = pd.DataFrame({'Date': indeedrealdatelist,'Job_Title': indeedtitlelist, 'URL' : indeedlinklist,'Company': indeedcompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : indeedcitylist, 'State': indeedstatelist, 'Department' : "Exchange"})
		...
		if len(linkedinlinklist) == len(linkedintitlelist) == len(linkedinlocationlist) == len(linkedincompanylist) == len(linkedindatelist):
			dflinkedin = pd.DataFrame({'Date': linkedindatelist,'Job_Title': linkedintitlelist, 'URL' : linkedinlinklist,'Company': linkedincompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : linkedincitylist, 'State': linkedinstatelist, 'Department': "Exchange"})
		...
		if len(monsterlinklist) == len(monstertitlelist) == len(monsterlocationlist) == len(monstercompanylist) == len(monsterrealdatelist):
			dfmonster = pd.DataFrame({'Date': monsterrealdatelist,'Job_Title': monstertitlelist, 'URL' : monsterlinklist,'Company': monstercompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : monstercitylist, 'State': monsterstatelist, 'Department': "Exchange"})
		...
		if len(simplylinklist) == len(simplytitlelist) == len(simplylocationlist) == len(simplycompanylist) == len(simplydatelist):
			dfsimply = pd.DataFrame({'Date': simplydatelist,'Job_Title': simplytitlelist, 'URL' : simplylinklist,'Company': simplycompanylist, 'Search_Parameter':searchval.replace("%20"," "), 'City' : simplycitylist, 'State': simplystatelist, 'Department': "Exchange"})
		...
		df.to_sql(con=database_connection, name='JobPostings_Exchange', if_exists='replace')
